...
class Reminders(Plugin):
    ...
    def check(self, command_obj):
        event_command = command_obj.text
        event_verbs = command_obj.verbs
    ...
    def exec(self, **kwargs):
        ...
        datetime_tz = kwargs["TimeZone"]
        session = kwargs["SessionData"]
        event_command = command_obj.text
        log.debug("In set a reminder with command {0}".format(event_command))
        event_ents = command_obj.ents
            ...
            response = {
                "errors":
                    [{
                        ...
                        "text": "Couldn't parse a time from {0}".format(event_time),
                        "id": "REMINDERS_TIME_INVALID"
                    }]
            }
            return response
        log.debug("Alert text is {0}".format(time_message))
        # Set the reminder using the events framework
        alert_time = time.time() + time_in_seconds
        log.debug(
            "Alert time is {0}, time is {1}, time_in seconds is {2}".format(alert_time, time.time(), time_in_seconds))
        session.notification(time_message, time.time()+time_in_seconds, title="Reminder")
        response = {
            "data":
                {
                    "type": "success",
                    "text": "Got it. I'll send you the following reminder: {0} {1} {2}".format(time_message,
                                                                                               time_word, event_time),
                    "id": "REMINDER_SUCCESSFUL"
                }
        }
        return response